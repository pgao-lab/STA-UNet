...
class KDloss(nn.Module):
    def __init__(self, lambda_x, adaptive_weights=True):
    ...
    def forward(self, feature, feature_decoder, final_up):
        ...
        encoder_weights = [0.5, 0.25, 0.15, 0.1] if self.adaptive_weights else [0.25] * 4
        decoder_weights = [0.4, 0.35, 0.25] if self.adaptive_weights else [0.33] * 3
        ...
        encoder_loss = sum(w * self.intra_fd(f) for f, w in zip([f1_0, f2_0, f3_0, f4_0], encoder_weights))

        decoder_loss = sum(w * self.intra_fd(f_d) for f_d, w in zip([f1_d_0, f2_d_0, f3_d_0], decoder_weights))

        inter_loss = (self.inter_fd(f1_d_0, final_layer) + self.inter_fd(f2_d_0, final_layer) + self.inter_fd(f3_d_0, final_layer) +
        ...
        loss = self.lambda_x * (encoder_loss + decoder_loss + inter_loss)
...
def trainer_synapse(args, model, snapshot_path):
    ...
    # from datasets.dataset_tcia import TCIA_dataset
    from datasets.dataset_brats import BraTs_dataset, RandomGeneratorBraTs
    from datasets.dataset_acdc import ACDC_dataset, RandomGenerator1
    ...
    if args.dataset == "Synapse":
        ...
        db_train = ACDC_dataset(args.root_path, args.list_dir, split="train", transform=
        transforms.Compose(
